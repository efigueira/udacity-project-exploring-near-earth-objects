...
class NearEarthObject:
    ...
    def __str__(self):
        """Return `str(self)`."""
        value = 'is' if self.hazardous else 'is not'
        return f"NEO {self.fullname} has a diameter of {self.diameter:.3f} km and {value} potentially hazardous."
    ...
    def serialize(self):
        ...
        :return: A dictionary representation of the `NearEarthObject`.
        """
        return {
...
class CloseApproach:
    ...
    def __str__(self):
        """Return `str(self)`."""
        return (f"At {self.time}, '{self.neo.fullname}' approaches Earth at a "
    ...
    def serialize(self, flatten: bool=False):
        """Serialize the `CloseApproach` object to a dictionary.

        :param flatten: If True, the NEO attributes are included in the top-level dictionary.
